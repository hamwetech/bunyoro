...
from conf.models import District, County, SubCounty, Parish, Village, PaymentMethod, MessageTemplates, Region
from conf.forms import DistrictForm, CountyForm, SubCountyForm, VillageForm, PaymentMethodForm, UploadLocation,\
MessageTemplatesForm, ParishForm, RegionForm
...
class RegionDeleteView(ExtraContext, DeleteView):
    model = Region
    success_url = reverse_lazy('conf:region_list')
...
class ParishListView(ExtraContext, ListView):
    ...
    def get_queryset(self):
        ...
        if village:
            queryset = queryset.filter(name=village)
        if subcounty:
...
class VillageListView(ExtraContext, ListView):
    ...
    def get_queryset(self):
        ...
        if village:
            queryset = queryset.filter(name=village)
        if subcounty:
            queryset = queryset.filter(parish__sub_county__name=subcounty)
        if county:
            queryset = queryset.filter(parish__sub_county__county__name=county)
        if district:
            queryset = queryset.filter(parish__sub_county__county__district__name=district)
        ...
        if form.is_valid():
            ...
            for i in range(startrow, sheet.nrows):
                try:
                    ...
                    if not re.search('^[A-Z0-9\s\(\)\-\.]+$', county, re.IGNORECASE):
                    ...
                    if not re.search('^[A-Z0-9\s\(\)\-\.]+$', sub_county, re.IGNORECASE):
                    ...
                    if not re.search('^[A-Z0-9\s\(\)\-\.]+$', parish, re.IGNORECASE):
